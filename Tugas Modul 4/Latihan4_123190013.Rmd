...
```
Perlu diingat bahwa beberapa negara bagian memiliki populasi dibawah 5 juta, sehingga untuk mempermudah analisis, buat plot dalam skala `log`. Transformasi nilai variabel menggunakan transformasi `log10`, kemudian tampilkan plot-nya.
```{r}
...
```{r}
hist(murders$population, col="cyan", main="Histogram Populasi Negara Bagian", xlab="Population")
```
9. Hasilkan *boxplot* dari populasi negara bagian berdasarkan wilayahnya.
```{r}
boxplot(population~region, data=murders, col=c("cyan"), cex.main=1.2, main="BOXPLOT Populasi Negara berdasarkan wilayah")
```